...
if page == "Create Client":
    ...
    with st.form("create_form", clear_on_submit=True):
        ...
        with c1:
            ...
            file = st.file_uploader("Leads (NAME, EMAIL, INFORMATION)", type=["csv", "xlsx"])
        with c2:
        ...
        if st.form_submit_button("Submit Client"):
            if name and file:
                ...
                save_data(); st.success(f"Client {name} stored!")
    ...
    for c_name, c_data in list(st.session_state.clients.items()):
        with st.expander(f"🏢 {c_name} | 👥 {len(c_data['leads'])} Leads"):
            ...
            with tab_edit:
                ...
                edit_desc = st.text_area("Context", value=c_data['desc'], key=f"ed_{c_name}")
                ...
                if st.button("Update Profile", key=f"up_p_{c_name}"):
            ...
            with tab_auto:
                ...
                # NEW: Edit Frequency after saving
                new_days = st.number_input("Days between emails", min_value=1, value=int(c_data.get('auto_days', 7)), key=f"day_{c_name}")
                if c_data['auto_on']:
            ...
            with tab_manual:
                ...
                framework = st.text_area("Framework", key=f"fr_{c_name}") if method == "Use Framework" else None
